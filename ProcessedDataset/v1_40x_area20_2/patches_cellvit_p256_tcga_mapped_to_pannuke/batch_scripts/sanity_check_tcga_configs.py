#!/usr/bin/env python3
"""
Sanity-check TCGA configs: print backbone, method, condition_source, film_target,
...
def extract_config_info(cfg: dict) -> dict:
    ...
    dataset = "TCGA" if "TCGA" in log_comment or "tcga" in str(data.get("dataset_path", "")).lower() else "?"
...
def main():
    ...
    print(f"{'Config':<45} {'backbone':<8} {'method':<10} {'cond':<8} {'film_tgt':<6} {'idinit':<6} {'lr':<12} {'seed':<5} {'dataset':<6}")
    print("-" * 120)
    ...
    for path in config_paths:
        ...
        print(f"{name:<45} {info['backbone']:<8} {info['method']:<10} {info['condition_source']:<8} {film_tgt:<6} {info['idinit']:<6} {str(info['lr']):<12} {str(info['seed']):<5} {info['dataset']:<6}")

    print("\nAll configs should have: backbone (Virchow/SAMH), method (Baseline/FiLM), condition_source (Rosie/None), idinit=true for FiLM, lr=3e-5, seed=19, dataset=TCGA")
    print("Run names (log_comment) should contain: backbone + TCGA + method + film_target + idinit + lr + seed for parse_run_name compatibility.")
