...
class CellViTInference:
    ...
    def _setup_worker(self) -> None:
        ...
        # Disable Ray parallelism on HPC; just run locally
        import ray
        ray.init(local_mode=True, num_cpus=1, runtime_env=runtime_env)

        # Force single worker to avoid CUDA contention
        self.num_workers = 1
        self.ray_actors = 1
        self.logger.info("Using 1 ray-worker (local mode)")
