    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.freeze_cellvit:
            print(f"❄️ Configuring SAM Backbone (LoRA={use_lora})...")
            for n, p in self.encoder.named_parameters():
                if use_lora and ("lora_" in n):
                    p.requires_grad = True # LoRA 必须训练
                else:
                    p.requires_grad = False # 其他参数冻结
        ...
        # FiLM module on z4 
        feat_dim = self.embed_dim # 自动获取 embed_dim (1280 for SAM-H)
