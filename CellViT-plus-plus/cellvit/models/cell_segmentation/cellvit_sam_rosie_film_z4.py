...
class CellViTSAMRosieFiLM(CellViTSAM):
    ...
    def __init__(
        ...
        # --- Optional: load ROSIE pretrained weights (domain-adapted) ---
        if rosie_weights_path is not None:
            ckpt = torch.load(rosie_weights_path, map_location="cpu")
            rosie_weights_key = None

            # handle common checkpoint formats
            if isinstance(ckpt, dict):
                if rosie_weights_key is not None and rosie_weights_key in ckpt:
                    state = ckpt[rosie_weights_key]
                else:
                    # try common keys, else assume dict is already state_dict
            ...
            # strip prefixes from DDP/lightning/etc.
            cleaned = {}
