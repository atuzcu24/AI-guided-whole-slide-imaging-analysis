...
from cellvit.models.cell_segmentation.cellvit_sam_lora import CellViTSAM
...
from cellvit.models.cell_segmentation.cellvit_sam_rosie_film_lora import CellViTSAMRosieFiLM #Fusion
...
from typing import Callable, Tuple, Union, List, Dict, Optional, Any
import copy
import datetime
import uuid
class ExperimentCellVitPanNuke(BaseExperiment):
    ...
    def get_train_model(
        ...
        **kwargs,
        ...
        # --- ✅ SAM-H with Rosie-FiLM (包含 LoRA 注入) ---
        if backbone_type.lower() == "sam-h-rosie-film":
            # 确保你已经正确修改了 cellvit_sam_rosie_film.py 并将其放在正确的路径
            from cellvit.models.cell_segmentation.cellvit_sam_rosie_film_lora import CellViTSAMRosieFiLM

            # 从配置中读取 LoRA 参数
            model_conf = self.run_conf.get("model", {})

            model = CellViTSAMRosieFiLM(
                model_path=pretrained_encoder,
                num_nuclei_classes=self.run_conf["data"]["num_nuclei_classes"],
                num_tissue_classes=self.run_conf["data"]["num_tissue_classes"],
                vit_structure="sam-h",  # 显式指定
                drop_rate=self.run_conf["training"].get("drop_rate", 0),
                regression_loss=regression_loss,
                rosie_hidden_dim=model_conf.get("rosie_hidden_dim", 256),
                
                # --- 传递 LoRA 参数 ---
                use_lora=model_conf.get("use_lora", False),
                lora_r=model_conf.get("lora_r", 16),
                lora_alpha=model_conf.get("lora_alpha", 32),
                lora_dropout=model_conf.get("lora_dropout", 0.1),
            ...
            # 注意：不要调用 model.freeze_encoder()，因为我们在 __init__ 里已经处理了 LoRA 的解冻逻辑
            # model.freeze_encoder() 

            self.logger.info("Loaded CellViT-SAM + Rosie-FiLM fusion model (sam-h backbone)")

            # --- LoRA 检查日志 ---
            self.logger.info("---- Trainable Parameters Check (LoRA Validation) ----")
            trainable_count = 0
            for name, p in model.named_parameters():
                if p.requires_grad:
                    trainable_count += 1
                    # 只打印前几个，确认是 encoder 里的 lora 参数
                    if trainable_count <= 3:
                        self.logger.info(f"✅ [TRAINABLE] {name}")
            
            if trainable_count == 0:
                self.logger.warning("⚠️ WARNING: No trainable parameters found! LoRA might not be active.")
            else:
                self.logger.info(f"🔥 LoRA Active! Total trainable parameter groups: {trainable_count}")
        ...
        try:
            ...
            self.logger.info("Could not print model summary (torchinfo might fail on complex models)")
